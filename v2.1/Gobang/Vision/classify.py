...
class Classify():
    ...
    def __init__(self, model_dir = 'Vision/model/model.ckpt', is_show=False):
        if is_show:
            print("载入模型")
    ...
    def chessidentify(self, filename, is_show = False):
        ...
        max_index = int(self.classes[index])
        print(max_index)
        prediction = prediction[0][index]

        if max_index == 2:
            #print('This is a red with possibility %.6f' %prediction[:, 0])
            result = 1 # 'red'
            temp_s = 'r'
        elif max_index == 1:
            #print('This is a green with possibility %.6f' %prediction[:, 1])
            result = 2 # 'green'
            temp_s = 'g'
        else :
            #print('This is a white with possibility %.6f' %prediction[:, 2])
            result = 0 # 'white'
            temp_s = 'k'
...
if __name__ == '__main__':
    img_list = [
        r"Vision\data\1\0005.jpg", r"Vision\data\2\0005.jpg",
        r"Vision\data\3\0005.jpg", r"Vision\pictures\data\red\red.0.jpg"
    ]
    ident = Classify()
