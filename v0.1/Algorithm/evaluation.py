...
class Evaluation (object):  
    ...
    def __init__ (self):  
        ...
        for i in range(N_board):  
            row = [(4 - max(abs(i - 4), abs(j - 4))) for j in range(N_board)]  
    ...
    def __analysis_horizon (self, board, i, j):  
        ...
        for x in range(N_board):  
            line[x] = board[i][x]  
        self.analysis_line(line, result, N_board, j)  
        for x in range(N_board):  
            if result[x] != TODO:  
    ...
    def __analysis_vertical (self, board, i, j):  
        ...
        for x in range(N_board):  
            line[x] = board[x][j]  
        self.analysis_line(line, result, N_board, i)  
        for x in range(N_board):  
            if result[x] != TODO:  
    ...
    def __analysis_left (self, board, i, j):  
        ...
        k = 0  
        while k < N_board:  
            if x + k > N_board - 1 or y + k > N_board - 1:  
                break  
    ...
    def __analysis_right (self, board, i, j):  
        ...
        record = self.record  
        TODO = self.TODO  
        if N_board - 1 - i < j: x, y, realnum = j - N_board + 1 + i, N_board - 1, N_board - 1 - i  
        else: x, y, realnum = 0, i + j, j  
        k = 0  
        while k < N_board:  
            if x + k > N_board-1 or y - k < 0:  
                break  
            line[k] = board[y - k][x + k]  
            k += 1  
        self.analysis_line(line, result, k, j - x)  
